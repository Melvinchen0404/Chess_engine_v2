...
def evaluate_mobility(board) -> str:
    ...
    # Count legal moves for white (before changing the turn)
    white_mobility = len(list(board.legal_moves)) if board.turn == chess.WHITE else 0
    
    # Switch to black's turn and count legal moves for black
    board.turn = chess.BLACK
    black_mobility = len(list(board.legal_moves))
    
    # Restore the turn to the original (white's turn)
